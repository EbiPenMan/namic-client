...
cc.Class({
    ...
    runEffect: function () {
        ...
        this._first_Init_Value.x = this.effectTaget.width;
        this._first_Init_Value.y = this.effectTaget.height;

        this._fill_Start_Values();
        this._fill_End_Values();

        this._startValueTemp = cc.v2(this._startValueRelative);
        this.effectTaget.width = this._startValueTemp.x;
        this.effectTaget.height = this._startValueTemp.y;
    ...
    _fill_Start_Values: function () {
        ...
        if (this.startValueType === baseEffectDefined.enValueTypeNoPredefined.Current) {
            ...
            this._startValueRelative.x = this.startValueNode.width;
            this._startValueRelative.y = this.startValueNode.height;
        }
        ...
        this.effectTaget.width = this._startValueRelative.x;
        this.effectTaget.height = this._startValueRelative.y;
    ...
    _fill_End_Values: function () {
        ...
        if (this.endValueType === baseEffectDefined.enValueTypeNoPredefined.Current) {
            ...
            this._endValueRelative.x = this.endValueNode.width;
            this._endValueRelative.y = this.endValueNode.height;
        }
        ...
        let Action = cc.sequence(
            ...
            cc.callFunc(function () {
                ...
                if (self.loopType === baseEffectDefined.enLoopType.Restart) {
                    self.effectTaget.width = self._startValueRelative.x;
                    self.effectTaget.height = self._startValueRelative.y;
                }
            }),

            //effect
            cc.sizeTo(self.effectDuration, self._endValueTemp.x, self._endValueTemp.y).easing(self._getCurrentEaseObject()),
    ...
    _reCalcValuesLoop: function () {
        ...
        if (self.loopType === baseEffectDefined.enLoopType.PingPong) {
            ...
            self._endValueTemp.x = self.effectTaget.width + self.endValue.x;
            self._endValueTemp.y = self.effectTaget.height + self.endValue.y;
        }
    },
